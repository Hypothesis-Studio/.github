# Hypothesis Studio

> Building tools that make developers more productive.

---

## About

**Hypothesis Studio** is an open-source organization focused on building modern developer tools. Our goal is to create fast, extensible, and thoughtfully designed software that developers actually enjoy using.

## 🚀 Projects

| Project | Description |
|---------|-------------|
| 📝 **[Hypothesis Editor](https://github.com/Hypothesis-Studio/Hypothesis-Editor)** | Create and edit Hypothesis projects with an intuitive interface. |
| 🧩 **[Hypothesis Extension](https://github.com/Hypothesis-Studio/Hypothesis-Extension)** | Official collection of extensions for Hypothesis Editor. |

---

## Contributing

We welcome contributions of all kinds — bug reports, feature requests, documentation, and code.

Check each repository's `CONTRIBUTING.md` for specific guidelines.

## Links

- 💬 [Discussions](https://github.com/orgs/Hypothesis-Studio/discussions/categories/general)
- 💡 [Request a Feature](https://github.com/orgs/Hypothesis-Studio/discussions/categories/ideas)
- 🐞 [Report a Bug](https://github.com/orgs/Hypothesis-Studio/discussions/categories/q-a)
- 🙏 [Q&A](https://github.com/orgs/Hypothesis-Studio/discussions/categories/q-a)

## License

All projects under Hypothesis Studio are licensed under the [MIT License](https://opensource.org/licenses/MIT).
